...
set "START_CMD=%POWERSHELL_PATH% -NoProfile -ExecutionPolicy Bypass -File \"%STARTER_PS%\""
...
if %errorLevel% neq 0 (
    type "%LOG_FILE%"
    echo [WARN] Failed to create service. Falling back to Task Scheduler startup.
    goto REGISTER_TASK
)
...
if %errorLevel% neq 0 (
    type "%LOG_FILE%"
    echo [WARN] Service failed to start. Falling back to Task Scheduler startup.
    sc delete "%SERVICE_NAME%" >nul 2>&1
    goto REGISTER_TASK
)
